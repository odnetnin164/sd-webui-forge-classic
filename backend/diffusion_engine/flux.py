...
class Flux(ForgeDiffusionEngine):
    ...
    def decode_first_stage(self, x):
        ...
        # Keep UNet loaded during VAE decoding to avoid unnecessary unload/reload cycles
        sample = self.forge_objects.vae.decode(sample, keep_loaded_models=[self.forge_objects.unet]).movedim(-1, 1) * 2.0 - 1.0
        return sample.to(x)
